...
- **Bảng điều khiển web:** mở domain Vercel (route `/`) để nhập token, tạo Codespace 6h, xem danh sách và đẩy backup thủ công trực tiếp từ trình duyệt (nhập `REPOSITORY_ID` và `BACKUP_GIST_ID` nếu chưa khai báo env).
- **API Vercel:** gọi các route `/api/codespace/*` với header `Authorization: Bearer <GITHUB_TOKEN>`
