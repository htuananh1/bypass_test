(() => {
  ...
  const AUTO_SELECTION_MAX_LENGTH = 1600;
  const STORAGE_KEYS = {
...
#aiPanelToggle.stealth.mobile {
  ...
  function attachEventHandlers() {
    ...
    document.addEventListener('touchend', handleSelectionInteraction, true);
    document.addEventListener('keyup', handleSelectionInteraction, true);
  ...
  function toggleTextMode() {
    ...
    setTextInputVisibility(!visible);
  }

  function setTextInputVisibility(visible) {
    ui.textInputSection.style.display = visible ? 'block' : 'none';
  }
  ...
  function handleSelectionChange() {
    ...
    const delay = state.isMobile ? 450 : 200;
  ...
  function handleSelectionInteraction(event) {
    if (state.selecting) return;
    if (isNodeInsideUi(event.target)) return;
    if (state.selectionTimer) {
      clearTimeout(state.selectionTimer);
      state.selectionTimer = null;
    }
    const delay = state.isMobile ? 260 : 80;
    window.setTimeout(() => {
      processSelection({ force: true });
    }, delay);
  }
  ...
  function processSelection(options = {}) {
    ...
    if (isSelectionInsideUi(selection)) {
      state.lastSelectionText = '';
      return;
    }
    ...
    if (text.length > AUTO_SELECTION_MAX_LENGTH) {
      setStatus('⚠️ Đoạn chọn quá dài, vui lòng thu gọn.', '#f1c40f');
      state.selectionCooldownUntil = Date.now() + 800;
      return;
    }
    const now = Date.now();
    if (state.lastSelectionText === text && now < state.selectionCooldownUntil && !options.force) {
      return;
    }
    state.lastSelectionText = text;
    state.selectionCooldownUntil = now + 1200;
    handleSelectedText(text);
  }

  function isSelectionInsideUi(selection) {
    if (!selection) return false;
    const nodes = [selection.anchorNode, selection.focusNode];
    return nodes.some((node) => isNodeInsideUi(node));
  }

  function isNodeInsideUi(node) {
    if (!node) return false;
    let current = node.nodeType === Node.TEXT_NODE ? node.parentElement : node;
    while (current) {
      if (current === ui.panel || current === ui.toggle || current === state.host) {
        return true;
      }
  ...
  function handleSelectedText(text) {
    ...
    ui.textQuestionInput.value = trimmed;
    clearPageSelection();
    sendQuestionText(trimmed);
  }
  ...
  function clearPageSelection() {
    ...
    try {
      ...
      // ignore inability to clear selection
    }
  }
  ...
  async function sendQuestionText(question) {
    ...
    await sendToGemini(`Câu hỏi: "${safeQuestion}".\n\n${prompt}`, null);
  }
  ...
  async function sendTextQuestion() {
    ...
    await sendQuestionText(question);
  }
