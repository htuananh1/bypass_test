...
const STORAGE_KEY = 'gptHomeworkSolver';
...
function bindEvents() {
  ...
  saveApiKeyBtn.addEventListener('click', () => {
    saveSettings({ apiKey: apiKeyInput.value.trim() })
      .then(() => setStatus('Đã lưu API key Vercel.', 'success'))
...
async function onSolveClick() {
  ...
  const cacheKey = await hashString(`${question}|${modelSelect.value}|${outputModeSelect.value}|${langSelect.value}|${customPromptInput.value.trim()}`);

  setLoading(true);
  setStatus('Đang gửi tới GPT...', 'warning');
...
function buildPrompt(question) {
  ...
  const sharedDirectives = [
    'Phân tích cẩn thận bài tập sau và tạo đáp án chính xác.',
    'Tự xác định môn học phù hợp rồi ghi rõ ở dòng đầu tiên theo định dạng "Môn: <tên môn>".',
    'Chỉ sử dụng dữ kiện được cung cấp, nêu rõ mọi giả định hoặc yêu cầu bổ sung nếu thông tin thiếu.',
    `Trả lời hoàn toàn bằng ${langName}, không thêm lời chào hoặc ký tên.`
  ];
  ...
  if (mode === 'custom') {
    ...
    modeDirective = `${customText}`;
  } else if (mode === 'answer') {
    modeDirective = 'Sau dòng ghi môn học, cung cấp duy nhất đáp án cuối cùng, không giải thích thêm.';
  } else {
    modeDirective = 'Sau dòng ghi môn học, trình bày lời giải từng bước: phân tích giả thiết, lập luận/ công thức, và kết luận cuối cùng rõ ràng.';
  }

  const numericDirective = 'Nếu câu trả lời có giá trị số, hãy ghi kèm đơn vị thích hợp và làm tròn hợp lý.';
