...
2. **Missing API key**: Mở popup khi chưa nhập key, trạng thái phải báo "Chưa có API key" và nút **Gửi GPT** trả về lỗi.
3. **Enter API key**: Dán key hợp lệ của Vercel AI Gateway, nhấn **Lưu API key**. Đóng/mở popup lại để chắc chắn trạng thái chuyển sang "Sẵn sàng".
4. **Selection sync**: Bôi đen đoạn văn bất kỳ trên trang, mở popup để kiểm tra ô "Văn bản được bôi đen" tự động điền nội dung.
5. **Solve request**: Để chế độ "Giải thích chi tiết", nhấn **Gửi GPT**. Kiểm tra Service Worker log xem request POST tới `https://ai-gateway.vercel.sh/v1/chat/completions`, popup hiển thị kết quả và câu trả lời có nêu rõ môn học được AI suy ra.
